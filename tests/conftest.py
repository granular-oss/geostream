...
TEST_DIR = os.path.abspath(os.path.split(os.path.split(__file__)[0])[0]) + "/tests"
TEST_FILES_DIR = TEST_DIR + "/test_files/"
TEST_OUT_DIR = TEST_DIR + "/_cli_out/"


def _make_feature(geom: dict, prop: dict):
    return dict(type="Feature", geometry=geom, properties=prop)


def _get_test_file(gjz_file: str):
    file_path = TEST_FILES_DIR + gjz_file
...
def feat_collection_1():
    feature_list = [_make_feature(geom=dict(type="Polygon",
                                            coordinates=[
...
@pytest.fixture(scope="function")
def feat_collection_2():
    feature_list = [_make_feature(geom=dict(type="Polygon", coordinates=[
                                                [[2.3812345673812914, 57.32238290582347], [23.194, -20.28],
                                                 [-120.43, 19.15], [2.3812345673812914, 57.32238290582347]]]),
                                  prop={"prop0": "val0"})]
    return dict(type="FeatureCollection", features=feature_list)


@pytest.fixture(scope="function")
def feat_collection_3():
    feature_list = [
        _make_feature(geom=dict(type="Polygon",
                                coordinates=[
                                    [[10.35126458923567, 10.35126458923569], [40, 10], [40, 40], [10, 40],
